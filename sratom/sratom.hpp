...
/// @file sratom.hpp C++ API for Sratom, a lightweight RDF syntax library
...
class Streamer
    : public serd::detail::BasicWrapper<SratomStreamer, sratom_streamer_free>
{
public:
	Streamer(serd::World& world, LV2_URID_Map& map, LV2_URID_Unmap& unmap)
	    : BasicWrapper(sratom_streamer_new(world.cobj(), &map, &unmap))
	{
	}

	int write(serd::SinkView sink, const LV2_Atom& atom, const Flags flags = {})
	{
		return sratom_stream(
		        cobj(), sink.cobj(), nullptr, nullptr, &atom, flags);
	}

	int write(serd::SinkView    sink,
	          const serd::Node& subject,
	          const serd::Node& predicate,
	          const LV2_Atom&   atom,
	          const Flags       flags = {})
	{
		return sratom_stream(cobj(),
		                     sink.cobj(),
		                     subject.cobj(),
		                     predicate.cobj(),
		                     &atom,
	...
	std::string
	to_string(serd::Env& env, const LV2_Atom& atom, const Flags flags = {})
	{
...
private:
	static size_t
	string_sink(const void* buf, size_t, size_t nmemb, void* stream)
	{
...
class Forger
    : public serd::detail::BasicWrapper<SratomForger, sratom_forger_free>
{
public:
	using AtomPtr = std::unique_ptr<LV2_Atom, detail::Deleter>;

	Forger(serd::World& world, LV2_URID_Map& map)
	    : BasicWrapper(sratom_forger_new(world.cobj(), &map))
	{
	}
	...
	AtomPtr read_string(serd::Env& env, const char* str, const Flags flags = {})
	{
		return AtomPtr{sratom_from_string(cobj(), env.cobj(), str, flags)};
	}
	...
	AtomPtr read_model(const serd::Optional<serd::Node>& base_uri,
	                   ...
	                   const Flags                       flags = {})
	{
		return AtomPtr{sratom_from_model(
		        cobj(), base_uri.cobj(), model.cobj(), subject.cobj(), flags)};
	}
};
